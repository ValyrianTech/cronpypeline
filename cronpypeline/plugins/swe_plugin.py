...
def _build_doc_sync_prompt(
    ...
    if pr_exists:
        push_instruction = (
            ...
            f"the existing PR:\n"
            f"  RunCommand: cd {shlex.quote(str(target_dir))} && git push origin {INTEGRATION_BRANCH}\n"
        )
    ...
    return (
        ...
        f"  cd {shlex.quote(str(target_dir))} && .venv/bin/pytest -q\n\n"
        f"Commit your changes on the `{INTEGRATION_BRANCH}` branch:\n"
        f"  cd {shlex.quote(str(target_dir))} && git add -A && "
        ...
        f"If `cd {shlex.quote(str(target_dir))} && git status --porcelain` shows no changes, "
...
def run_a9_dep_audit(action: ActionSpec, context: TickContext) -> ActionResult:
    ...
    if issues_created > 0 or vulnerabilities:
        # Issues were created, OR the audit found vulnerabilities that were
        # all either tooling packages or already represented by existing issues.
        # Either way, the audit's purpose was fulfilled.
        result.success = True
    else:
        # Audit failed and no vulnerabilities were parsed from the output -
