...
def _build_review_prompt(repo_dir: Path, repo_name: str, task_dir: Path,
    ...
    return (
        ...
        f"## The task\n\n"
        f"{issue.body.strip()}\n\n"
        f"## Step 1 — plan with the Progress tool\n\n"
        f"Before you start reviewing, use the **Progress** tool to record "
        f"your review plan. Keep it updated as you go.\n\n"
        f"## Step 2 — review the codebase\n\n"
        f"Use ReadFile to examine source files across the repo. Focus on:\n"
        f"- **Correctness**: Logic errors, off-by-one bugs, race conditions\n"
        f"- **Reliability**: Missing error handling, resource leaks, unsafe patterns\n"
        ...
        f"Write a completion marker summarizing your review (how many issues "
        f"filed, key areas covered) into this file:\n"
        f"     {marker_path}\n\n"
        f"CRITICAL: The pipeline CANNOT proceed until you write this marker. "
        f"Write it EVEN IF you found no issues (summarize that the codebase "
        f"looks clean). The pipeline needs it to advance.\n\n"
