...
class TestLoadGithubTokenDotenvFallback:
    def test_loads_token_from_dotenv(self, tmp_path, monkeypatch):
        ...
        def dotenv_values(env_file):
            return {"SWE_GITHUB_TOKEN": "dotenv-token"}
        ...
        assert "SWE_GITHUB_TOKEN" not in os.environ
    ...
    def test_load_env_token_does_not_mutate_os_environ(self, tmp_path, monkeypatch):
        monkeypatch.delenv("SWE_GITHUB_TOKEN", raising=False)
        env_file = tmp_path / ".env"
        env_file.write_text('SWE_GITHUB_TOKEN="quoted-token"\n', encoding="utf-8")
        with patch.dict("sys.modules", {"dotenv": None}):
            token = _load_env_token(env_file, ("SWE_GITHUB_TOKEN", "GITHUB_TOKEN"))
        assert token == "quoted-token"
        assert "SWE_GITHUB_TOKEN" not in os.environ

    def test_load_env_token_prefers_first_truthy_key(self, tmp_path):
        env_file = tmp_path / ".env"
        env_file.write_text("GITHUB_TOKEN=gh-only\n", encoding="utf-8")
        with patch.dict("sys.modules", {"dotenv": None}):
            assert _load_env_token(env_file, ("SWE_GITHUB_TOKEN", "GITHUB_TOKEN")) == "gh-only"
        with patch.dict("sys.modules", {"dotenv": None}):
            assert _load_env_token(env_file, ("MISSING_TOKEN",)) is None

    def test_load_env_token_uses_dotenv_values(self, tmp_path):
        env_file = tmp_path / ".env"
        env_file.write_text("SWE_GITHUB_TOKEN=dotenv-token\n", encoding="utf-8")
        import types
        dotenv_mod = types.ModuleType("dotenv")
        dotenv_mod.dotenv_values = lambda p: {"SWE_GITHUB_TOKEN": "dotenv-token"}
        with patch.dict("sys.modules", {"dotenv": dotenv_mod}):
            assert _load_env_token(env_file, ("SWE_GITHUB_TOKEN", "GITHUB_TOKEN")) == "dotenv-token"
    ...
    def test_parse_env_file(self, tmp_path):
        ...
        env_file.write_text(
            ...
            "1INVALID=bad\n"
            "INVALID-KEY=bad\n"
            'SWE_GITHUB_TOKEN="double-quoted"\n'
            "OTHER_VAR='single-quoted'\n"
            "PLAIN_VAR=plain-token\n"
            "EMPTY=\n",
            encoding="utf-8",
    ...
    def test_does_not_read_env_from_great_great_grandparent(self, tmp_path, monkeypatch):
        monkeypatch.delenv("SWE_GITHUB_TOKEN", raising=False)
        monkeypatch.delenv("GITHUB_TOKEN", raising=False)
        from cronpypeline.plugins import swe_plugin
        swe_dir = tmp_path / "workspace" / "tasks" / "x"
        ...
        # 4+ levels up must NOT be read
        great_great_grandparent_env = swe_dir.parent.parent.parent.parent / ".env"
        great_great_grandparent_env.write_text("SWE_GITHUB_TOKEN=great-great-grandparent-token\n")
        with patch.dict("sys.modules", {"dotenv": None}):
            assert _load_github_token({}) is None
    ...
    def test_does_not_read_env_from_grandparent(self, tmp_path, monkeypatch):
        monkeypatch.delenv("SWE_GITHUB_TOKEN", raising=False)
        monkeypatch.delenv("GITHUB_TOKEN", raising=False)
        monkeypatch.delenv("SWE_ENV_FILE", raising=False)
        from cronpypeline.plugins import swe_plugin
        swe_dir = tmp_path / "workspace" / "tasks" / "x"
        swe_dir.mkdir(parents=True)
        monkeypatch.setattr(swe_plugin, "SWE_WORKSPACE_DIR", swe_dir)
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "home")
        # SWE_WORKSPACE_DIR.parent.parent / ".env" (2 levels up) is NOT read
    ...
    def test_does_not_read_env_from_great_grandparent(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(swe_plugin, "SWE_WORKSPACE_DIR", swe_dir)
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "home")
        # SWE_WORKSPACE_DIR.parent.parent.parent / ".env" (3 levels up) is NOT read
    ...
    def test_does_not_read_env_from_cwd(self, tmp_path, monkeypatch):
        ...
        (cwd / ".env").write_text("SWE_GITHUB_TOKEN=vulnerable-token\n")
        monkeypatch.setattr(Path, "cwd", lambda: cwd)
        with patch.dict("sys.modules", {"dotenv": None}):
            assert _load_github_token({}) is None

    def test_home_dotenv_not_used_when_workspace_dotenv_absent(self, tmp_path, monkeypatch):
        monkeypatch.delenv("SWE_GITHUB_TOKEN", raising=False)
        monkeypatch.delenv("GITHUB_TOKEN", raising=False)
        monkeypatch.delenv("SWE_ENV_FILE", raising=False)
        from cronpypeline.plugins import swe_plugin
        ...
        (home / ".env").write_text("SWE_GITHUB_TOKEN=home-token\n")
        monkeypatch.setattr(Path, "home", lambda: home)
        with patch.dict("sys.modules", {"dotenv": None}):
            assert _load_github_token({}) is None

    def test_workspace_dotenv_wins_home_dotenv_ignored(self, tmp_path, monkeypatch):
        monkeypatch.delenv("SWE_GITHUB_TOKEN", raising=False)
        monkeypatch.delenv("GITHUB_TOKEN", raising=False)
        monkeypatch.delenv("SWE_ENV_FILE", raising=False)
        from cronpypeline.plugins import swe_plugin
        swe_dir = tmp_path / "workspace" / "tasks" / "x"
        swe_dir.mkdir(parents=True)
        (swe_dir / ".env").write_text("SWE_GITHUB_TOKEN=workspace-token\n")
        monkeypatch.setattr(swe_plugin, "SWE_WORKSPACE_DIR", swe_dir)
        home = tmp_path / "home"
        home.mkdir(parents=True)
        (home / ".env").write_text("SWE_GITHUB_TOKEN=home-token\n")
    ...
    def test_workspace_dotenv_loaded_and_parent_dirs_ignored(self, tmp_path, monkeypatch):
        ...
        swe_dir.mkdir(parents=True)
        (swe_dir / ".env").write_text("SWE_GITHUB_TOKEN=workspace-token\n")
        (swe_dir.parent.parent / ".env").write_text("SWE_GITHUB_TOKEN=grandparent-token\n")
        (swe_dir.parent.parent.parent / ".env").write_text("SWE_GITHUB_TOKEN=great-grandparent-token\n")
        monkeypatch.setattr(swe_plugin, "SWE_WORKSPACE_DIR", swe_dir)
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "home")
        with patch.dict("sys.modules", {"dotenv": None}):
            assert _load_github_token({}) == "workspace-token"

    def test_does_not_read_env_from_4_levels_up(self, tmp_path, monkeypatch):
        monkeypatch.delenv("SWE_GITHUB_TOKEN", raising=False)
        monkeypatch.delenv("GITHUB_TOKEN", raising=False)
        monkeypatch.delenv("SWE_ENV_FILE", raising=False)
        from cronpypeline.plugins import swe_plugin
        swe_dir = tmp_path / "workspace" / "tasks" / "x"
        swe_dir.mkdir(parents=True)
        (swe_dir.parent.parent.parent.parent / ".env").write_text("SWE_GITHUB_TOKEN=great-great-grandparent-token\n")
        monkeypatch.setattr(swe_plugin, "SWE_WORKSPACE_DIR", swe_dir)
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "home")
        with patch.dict("sys.modules", {"dotenv": None}):
            assert _load_github_token({}) is None

    def test_swe_env_file_override_used(self, tmp_path, monkeypatch):
        monkeypatch.delenv("SWE_GITHUB_TOKEN", raising=False)
        monkeypatch.delenv("GITHUB_TOKEN", raising=False)
        monkeypatch.delenv("SWE_ENV_FILE", raising=False)
        from cronpypeline.plugins import swe_plugin
        ...
        override.write_text("SWE_GITHUB_TOKEN=override-token\n")
        monkeypatch.setenv("SWE_ENV_FILE", str(override))
        with patch.dict("sys.modules", {"dotenv": None}):
            assert _load_github_token({}) == "override-token"

    def test_swe_env_file_takes_precedence_over_workspace_dotenv(self, tmp_path, monkeypatch):
        monkeypatch.delenv("SWE_GITHUB_TOKEN", raising=False)
        monkeypatch.delenv("GITHUB_TOKEN", raising=False)
        monkeypatch.delenv("SWE_ENV_FILE", raising=False)
        from cronpypeline.plugins import swe_plugin
    ...
    def test_swe_env_file_empty_ignored(self, tmp_path, monkeypatch):
        monkeypatch.delenv("SWE_GITHUB_TOKEN", raising=False)
        monkeypatch.delenv("GITHUB_TOKEN", raising=False)
        monkeypatch.delenv("SWE_ENV_FILE", raising=False)
        from cronpypeline.plugins import swe_plugin
        ...
        (swe_dir / ".env").write_text("SWE_GITHUB_TOKEN=workspace-token\n")
        monkeypatch.setattr(swe_plugin, "SWE_WORKSPACE_DIR", swe_dir)
        monkeypatch.setenv("SWE_ENV_FILE", "")
        with patch.dict("sys.modules", {"dotenv": None}):
            assert _load_github_token({}) == "workspace-token"

    def test_swe_env_file_nonexistent_ignored(self, tmp_path, monkeypatch):
        monkeypatch.delenv("SWE_GITHUB_TOKEN", raising=False)
        monkeypatch.delenv("GITHUB_TOKEN", raising=False)
        monkeypatch.delenv("SWE_ENV_FILE", raising=False)
        from cronpypeline.plugins import swe_plugin
        ...
        swe_dir.mkdir(parents=True)
        monkeypatch.setattr(swe_plugin, "SWE_WORKSPACE_DIR", swe_dir)
        monkeypatch.setenv("SWE_ENV_FILE", str(tmp_path / "does-not-exist.env"))
        with patch.dict("sys.modules", {"dotenv": None}):
            assert _load_github_token({}) is None
