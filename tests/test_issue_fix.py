...
class TestPromptBuilders:
    ...
    def test_review_prompt_quotes_repo_dir_with_special_chars(self, tmp_path):
        td = tmp_path / "t"; td.mkdir()
        repo_dir = tmp_path / "repo;rm -rf /"
    ...
    def test_closing_loop_quotes_repo_dir_with_special_chars(self, tmp_path):
        td = tmp_path / "t"; td.mkdir()
        repo_dir = tmp_path / "repo;rm -rf /"
...
class TestRunGate:
    ...
    def test_baseline_checkout_integration_fails(self, tmp_path, capsys):
        t = self._setup_git_with_branch(tmp_path)
        (t / "fix.txt").write_text("f")
        subprocess.run(["git", "-C", str(t), "add", "-A"], capture_output=True, check=True)
        subprocess.run(["git", "-C", str(t), "commit", "-m", "f"], capture_output=True, check=True)
    ...
    def test_baseline_checkout_fail_rejects_coverage_regression(self, tmp_path):
        t = self._setup_git_with_branch(tmp_path)
        (t / "fix.txt").write_text("f")
        subprocess.run(["git", "-C", str(t), "add", "-A"], capture_output=True, check=True)
        subprocess.run(["git", "-C", str(t), "commit", "-m", "f"], capture_output=True, check=True)
...
class TestIsQueueEmpty:
    ...
    def test_existing_nonempty_dir(self, tmp_path):
        ctx = _make_tick_context(tmp_path)
        ctx.pipeline = MagicMock()
        queue_dir = tmp_path / "queue"
        queue_dir.mkdir()
