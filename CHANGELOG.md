...
- The issue store's `create_issue()` now sanitizes the issue ID before using it as a filename: any character that is not alphanumeric, dot, underscore, or hyphen is replaced with `-` (leading/trailing dashes stripped), and an empty sanitized ID falls back to `issue`. The resolved path is validated with `.resolve()`/`is_relative_to()`; a filename that would still escape the issues directory raises a `ValueError`. This prevents path traversal attacks via malicious issue IDs (e.g. `../../evil` or absolute paths like `/etc/passwd`).
