...
HTTP redirects are followed manually (up to 5 hops), and SSRF protection is re-validated on every redirect hop — the URL scheme (http/https only) and private-IP check are applied to each hop before it is followed. This prevents an SSRF bypass where an attacker redirects a request to a private/reserved IP (e.g. `localhost`, `169.254.169.254`) after the initial URL passed validation. When a redirect goes to a different host than the original URL, the `Authorization` header is stripped from the redirected request so the auth token is never leaked to a cross-host redirect target; redirects that stay on the same host keep the `Authorization` header. Redirects without a `Location` header return an error, and exceeding the 5-hop limit returns `"Too many redirects (max 5)"`. For POST requests, 301/302/303 redirects switch to GET (matching urllib's default behavior), while 307/308 redirects for POST are not followed.
