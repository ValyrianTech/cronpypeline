...
- `resolve_private_ip`: boolean (default `true`). The handler always resolves the hostname and blocks requests that resolve to private/reserved IPs (SSRF protection). When `true` (default), the connection is also pinned to the validated public IP addresses to prevent DNS rebinding attacks. Set to `false` to skip IP pinning (the connection uses normal DNS resolution at connect time), but private/reserved IPs are still blocked.
