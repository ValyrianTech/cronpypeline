...
The SWE plugin's GitHub token resolution (`_load_github_token`) searches for a `.env` file only in the pipeline's workspace directory (`SWE_WORKSPACE_DIR/.env`) and the user's home directory (`~/.env`) when falling back to a dotenv file, rather than searching arbitrary parent directories of the workspace or the current working directory. This prevents credentials from being loaded from an untrusted current working directory or an unexpected parent directory.
